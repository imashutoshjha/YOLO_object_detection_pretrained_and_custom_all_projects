...
img_to_detect=cv2.imread('images/p5.webp')
...
for max_valueid in max_value_ids:
    ...
    img_to_crop=cv2.imread('images/p5.webp')#we have read image everytime because we don't want boundary of a cropped image on another.
    crop_img=img_to_crop[start_y_pt:end_y_pt,start_x_pt:end_x_pt]
    cv2.imwrite("crops/object"+str(i)+".jpg",crop_img)
    i=i+1

    #To extract number from number plate
    from PIL import Image
    from pytesseract import image_to_string
    text = image_to_string(crop_img,config ='--psm 6')
    print(text)
